...
#Lorem Ipsum change for GitHub project... just some more changes


# I changed teh strcture as referenced in "Suggestions to Make Your Project Stand Out!" in the project specifications
    # the code now runs the mathematical part (analysis part) in one step, instead in every step. I thought this maybe saves time calculating, but I got no reference value, but the reviser can tell me.
# trial and error'd with the time (datetime and timedelta), so I checked several StackOverflow postings, and (finally) found the datetime package.
# sometimes the (calculating) time is 0.0, which cannot be right...
...
def popular_trip(df):
    ...
    # The 'journey' column is created in the analysis() function.
...
def display_data(df):
    ...
    if display.lower() == 'yes':
        # prints every column except the 'journey' column created in analysis()
...
def analysis():
    '''Calculates and prints out the descriptive analysis about a city and
